...
func AddHash(prop *define.AddHash) error {
	...
	// 判断key是否已存在
	_, err = rdb.HMGet(context.Background(), prop.Key).Result()
	if err != redis.Nil {
		return errors.New("key值已存在")
	}
	// 验证通过，设置key
	err = rdb.HMSet(context.Background(), prop.Key, prop.Value, prop.TTL).Err()
	if err != nil {
		return err
	}
	return nil
...
func DelHash(prop *define.DelString) error {
	// 非空判断
	if prop.ConnIdentity == "" {
		return errors.New("唯一标识不能为空")
	}
...
func UpdateHash(prop *define.AddUpdateString) error {
	// 非空判断
	if prop.ConnIdentity == "" {
		return errors.New("唯一标识不能为空")
	}
	...
	// 验证通过，设置key
	err = rdb.Set(context.Background(), prop.Key, prop.Value, 0).Err()
	if err != nil {
		return err
	}
	return nil
